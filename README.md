...
| 模拟训练 | — | **时光机模拟驾驶舱**:回到历史任意一天盲盒炒股,未来数据服务端级遮蔽(见下文招牌功能) |
...
| 📈 交易 | **时光机** / 交易 / 回测实验室 / 永久组合 | **时光机:历史盲盒模拟炒股,练盘感不伤钱包**;多策略回测、参数优化、有效前沿、walk-forward 验证、业绩归因 |
| 🧮 分析 | 分析 / 看板 / 风险 / 预警 | 持仓收益统计、多股对比、月度热力图、相关性矩阵、风险指标 |
| 🤖 AI | AI 交易助手 / 设置 / 日志 | OpenAI 兼容协议接入,盘中辅助分析、宏观解读 |

### 招牌功能

![时光机模拟驾驶舱](docs/screenshots/replay-cockpit.png)

*时光机驾驶舱实拍:K 线风挡只放到"今天"(未来被服务端截断遮蔽),右栏是下单台与指标仪表,底部油门控制台支持 1x/2x/4x 自动播放*

- **✈ 时光机模拟驾驶舱**(本版主角):把任意一天变成"今天"。选定历史日期与初始资金进入驾驶舱——**未来 K 线由服务端逐日截断供给,翻不到明天**;每天看到的是当日真实盘面、指标与估值分位,买卖按拟真规则撮合(尾盘价成交、T+1、涨跌停分档拒单、买入整手、佣金印花税按 2023 年新政换挡)。选股也是当时口径:财报按"报告期滞后 60 天"生效(2020 年 3 月只看得到 2019 三季报),还有当日涨幅/成交额榜可用。支持 1x/2x/4x 自动播放;随时「揭晓复盘」——你的买卖点标注在完整 K 线上,净值曲线对比沪深 300,胜率/最大回撤/年化/超额收益一次算清。练盘感,不伤钱包。
...
- **国家队追踪**:中央汇金、证金等"国家队"资金动向全景仪表盘——ETF 持仓测算、重仓股名单与变动、增持减持榜单一屏看全,跟着"聪明钱"的逆周期调节节奏走
